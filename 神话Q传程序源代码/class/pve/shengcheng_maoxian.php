...
$resultl = mysqli_query($db,"SELECT * FROM guaiwu WHERE userid='".$userid."' and leixing='maoxian' and map='".$map[id]."' and qixue>'0'");
$guaiwu = mysqli_fetch_array($resultl);
if (!$guaiwu){
    if(!$_SESSION['guaiwumaoxian']){//不存在数量生成执行生成数量
 $guaiwu=array();
...
$guaiwu=(array_count_values($guaiwu));
    	...
    	$guaiwu=(array_count_values($_SESSION['guaiwumaoxian']));
    }
...
$s="insert into guaiwu(yuanshi,userid,leixing,username,text,buzhuo,map,chengzhanglv,dengji,dengji_max,zhongzu,qixue,qixuemax,fali,fali_max,fangyu,gongji_fa,gongji,sudu,jineng,pojia,mianshang) values('".$guaiwuid."','".$userid."','maoxian','".$name.$j."','".$text."','".$muban[buzhuo]."','".$map[id]."','".$chengzhanglv."','".$dengji."','".$dengji_max."','".$zhongzu."','".$qixue."','".$qixue_max."','".$fali."','".$fali_max."','".$fangyu."','".$fagong."','".$wugong."','".$sudu."','".$guaiwujinengs[$guaiwujineng]."','".$muban[pojia]."','".$muban[mianshang]."')";
$ok=mysqli_query($db,$s);
}
}
}

unset($_SESSION['guaiwumaoxian']);