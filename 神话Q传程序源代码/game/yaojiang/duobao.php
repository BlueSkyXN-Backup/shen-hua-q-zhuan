...
$jiangli="196|168|166|188";//中奖号码
$jiangli_wupin="wp,482|shenzhoubi,2888|wp,482|wp,482";//中奖奖品
$jiangli_wupin_shuliang="1,1|2888,2888|1,1|1,1";//中奖奖品数量
...
echo $huode_jiangli= $xyz->beibao($jiangli_wupin_id[$i],$jiangli_wupin_shuliang_id[$i],$jiangli_wupin_jilv_id[$i],10,$userid,'','');
	$s="insert into news(text,time,userid) values('鸿运当头，神州币夺宝中获得【".$huode_jiangli."】幸运号码。','".$pass."','$userid')";
$ok=mysqli_query($db,$s);
	mysqli_query($db,"COMMIT");
	echo'<br/><a href="duobao">继续摇奖</a><br/>';
	break;//跳出循环
}else{
	if($i==$chou){
	echo'获得号码：'.$suiji.'！非常遗憾！你没有中奖。<br/>
		<a href="duobao">继续夺宝</a><br/>';
	}
}
...
$ok=mysqli_query($db,$s);
	...
	echo"抽奖失败<br/><a href='duobao'>继续夺宝</a><br/>";
mysqli_query($db,"ROLLBACK");
}
}else{
	echo"你没有足够的神州币<br/><a href='duobao'>继续夺宝</a><br/>";
mysqli_query($db,"ROLLBACK");
}
mysqli_query($db,"END");
mysqli_query($db,"SET AUTOCOMMIT=1");//自动提交
    ...
    echo"-------------------------------<br/><img src='/img/yao.png'height='100'/><br/>
		...
		<a href='duobao?mys=ududhduhwuduwbbdwffwf'>点击夺宝</a>(需要神州币*2888)
		<br/>-------------------------------<br/>";
        //解读充值奖励	
