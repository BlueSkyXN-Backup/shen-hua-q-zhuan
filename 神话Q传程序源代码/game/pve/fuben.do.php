...
$pvp_leixing="fuben";
if($user['mys']!="fuben"){
    echo "野怪被偷走了~<a href='/fuben/map'>返回副本</a>！<br/>";
...
if(!$fuben){
	header("location:../fuben/map");//跳转地址
}
	$muban = mysqli_query($db,"SELECT * FROM muban_fuben WHERE id='".$fuben[leibie]."'");
$muban = mysqli_fetch_array($muban);
$guanqia = explode("|", $muban[xiaoguo]); 	//获取模板数据
$guan=$fuben[guan]-1;
$guanqia_shuju= explode(",", $guanqia[$guan]);
if($fuben[guaiwushu]>=$guanqia_shuju['1']){
    echo"击杀达标";
    header("location:../fuben/map");//跳转地址
    exit();
}
$wg_leixing="fuben";
$pk[id]=$userid.$user[map]."fuben";
//战斗操作
...
$result = mysqli_query($db,"SELECT * FROM guaiwu WHERE  userid='".$userid."' and map='".$fuben[id]."' and leixing='".$wg_leixing."' and qixue>'0'");
...
if($_GET['md5']){
	...
	$result = mysqli_query($db,"SELECT * FROM guaiwu WHERE userid='".$userid."' and leixing='".$wg_leixing."' and map='".$fuben[id]."'and qixue>'0'");
while($row = mysqli_fetch_array($result))
  {
...
<a href='/pve/pve.do?GO=fuben'>继续打怪</a> <br/>
...
if($fuben[guan]==$map2[guan]){
	      $guanqia_shuju2= explode("-", $guanqia_shuju[0]);
	if($guanqia_shuju2[0]==$guaiwu_yuanshi[$jl]){
	$map2[guaiwushu]+=1;
	   $sql1="update fuben set guaiwushu='".$map2[guaiwushu]."'where duiwuid='".$user[duiwu_id]."'";
...
//获取当前地图怪物数据
$result = mysqli_query($db,"SELECT * FROM guaiwu WHERE userid='".$user['id']."' and map='".$fuben[id]."' and leixing='".$wg_leixing."' and qixue>'0'");
$zhandou_guaiwu=NULL;
while($row = mysqli_fetch_array($result)){
...
$yaopin_html=yaopin(kj1,$user[kj1],fuben)."|".yaopin(kj2,$user[kj2],fuben)."|".yaopin(kj3,$user[kj3],fuben)."|".yaopin(kj4,$user[kj4],fuben)."|".yaopin(kj5,$user[kj5],fuben);
$jineng_html=jineng(kj6,$user[kj6],$user[zhongzu],fuben)."|".jineng(kj7,$user[kj7],$user[zhongzu],fuben)."|".jineng(kj8,$user[kj8],$user[zhongzu],fuben)."|".jineng(kj9,$user[kj9],$user[zhongzu],fuben)."|".jineng(kj10,$user[kj10],$user[zhongzu],fuben);
...
$zidong<a href='fuben.do?go=bz&md5=$_G[md5]'>捕捉</a> <a href='fuben.do?go=tp&md5=$_G[md5]'>逃跑</a> 
<br/>技能：<a href='fuben.do?go=putong&md5=$_G[md5]'>普通</a>$jineng_html
