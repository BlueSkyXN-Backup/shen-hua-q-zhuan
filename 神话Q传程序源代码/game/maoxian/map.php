...
$dakai_baoxiang=mysqli_query($db,"SELECT * FROM baoxiang WHERE id='".$baoxiang."' and leibie='maoxian' and map='".$map[id]."'");
...
mysqli_query($db,"SET AUTOCOMMIT=1");
		...
		//获取模板
		$muban = mysqli_query($db,"SELECT * FROM muban_maoxian WHERE id='".$map[leibie]."'");
...
if($map[guaiwushu]>=$guanqia_shuju['1']){
	...
	$map[guan]+=1;
	$map[guaiwushu]=0;
		   $sql1="update maoxian set guan='".$map[guan]."',guaiwushu='".$map[guaiwushu]."' where duiwuid='".$user[duiwu_id]."'";
$ok1=mysqli_query($db,$sql1);
$sql3 = "delete from baoxiang where leibie='maoxian' and map='".$map[id]."'";
...
{
 $resultl = mysqli_query($db,"SELECT * FROM baoxiang WHERE muban='".$guanqia_baoxiang[$bx]."' and leibie='maoxian' and map='".$map[id]."'");
...
$s="insert into baoxiang(muban,map,name,leibie) values('".$guanqia_baoxiang[$bx]."','".$map[id]."','".$baoxiangname[name]."','maoxian')";
$ok=mysqli_query($db,$s);
...
case"jifen":
$row[maoxian] +=$suiji;
$sql2="update users set maoxian='".$row[maoxian]."' where id='".$row[id]."'";
$ok=mysqli_query($db,$sql2);
...
$s="insert into email(text,userid,leibie,zhuangtai) values('你通关冒险，获得奖励：".$huode_html."。','".$row[id]."','1','0')";
...
mysqli_query($db,"SET AUTOCOMMIT=1");
	...
	//删除冒险数据
		$sql3 = "delete from maoxian where duiwuid='".$user[duiwu_id]."'";
$ok1=mysqli_query($db,$sql3);
$sql3 = "delete from baoxiang where leibie='maoxian' and map='".$map[id]."'";
$ok1=mysqli_query($db,$sql3);
echo"<br/>你已经成功通关冒险！<br/><a href='/maoxian/index?id=$map[id]'>返回冒险首页</a><br/>";
...
$ok1=mysqli_query($db,$sql3);
	$result = mysqli_query($db,"select * from users WHERE duiwu_id='".$duiwu[id]."'");
while($row = mysqli_fetch_array($result))
  {$s="insert into email(text,userid,leibie,zhuangtai) values('成功退出冒险！','".$row[id]."','1','0')";
$ok5=mysqli_query($db,$s);}

echo"冒险退出成功！<a href='/maoxian/index?id='".$zhuangtai_map."'>返回冒险首页</a><br/>";	}
...
echo "第".$map[guan]."关，击杀$muban_guaiwu[name] ($map[guaiwushu],$guanqia_shuju[1] )  <a href='/maoxian/map?id=$map[id]'>刷新</a><br/>";
	if($map[time]<time()){
	echo"冒险即将自动退出<br/>";	
	$sql3 = "delete from maoxian where duiwuid='".$user[duiwu_id]."'";
$ok1=mysqli_query($db,$sql3);
	$result = mysqli_query($db,"select * from users WHERE duiwu_id='".$duiwu[id]."'");
while($row = mysqli_fetch_array($result))
  {$s="insert into email(text,userid,leibie,zhuangtai) values('你未在规定时间内通关冒险已经自动退出！','".$row[id]."','1','0')";
$ok5=mysqli_query($db,$s);}
	}else{
$map[time]-=time();
$zaixian_time1=timesecond($map[time]);
echo"冒险将在".$zaixian_time1."之后自动<a href='map?tuichu=dfdsdddv'>退出</a>";
}
echo "<br/>你看见了：";
...
$_SESSION['guaiwumaoxian']=$guaiwu;
 ...
 $rows = mysqli_fetch_array($result);
   $guaiwu_map.="<a href='/pve/pve.do?GO=maoxian'>".$rows[name]."</a>(".$guaiwu_b.")";
 }
if($guaiwu_map){
 $guaiwu_map="<br/>【怪物】".$guaiwu_map."";
}
//获取当前地图宝箱
$resultl = mysqli_query($db,"SELECT * FROM baoxiang WHERE leibie='maoxian' and map='".$map[id]."'");
$my = mysqli_fetch_array($resultl);
if($my){
$maps_baoxiang= "<br/>【宝箱】";
}
$result = mysqli_query($db,"SELECT * FROM baoxiang WHERE leibie='maoxian' and map='".$map[id]."'");
while($row = mysqli_fetch_array($result))
  {
$maps_baoxiang.= "<a href='/maoxian/map?id=$map[id]&baoxiang=$row[id]'><img src='/img/ico/".$row[ico]."'/>$row[name]</a> ";
  }
