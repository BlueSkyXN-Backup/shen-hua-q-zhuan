...
$pvp_leixing="pvp";
if($user['mys']!="pve"){
    echo "野怪被偷走了~<a href='/map.games'>返回地图</a>！<br/>";
     mysqli_query($db,"update users set mys='map' where id='".$userid."'");
       exit();
}

$wg_leixing="guaiwu";
$pk[id]=$userid.$user[map]."guaiwu";
//战斗操作
...
$result = mysqli_query($db,"SELECT * FROM guaiwu WHERE  userid='".$userid."' and map='".$user['map']."' and leixing='".$wg_leixing."' and qixue>'0'");
...
if($_GET['md5']){
	...
	$result = mysqli_query($db,"SELECT * FROM guaiwu WHERE userid='".$userid."' and leixing='".$wg_leixing."' and map='".$user['map']."'and qixue>'0'");
while($row = mysqli_fetch_array($result))
  {
...
//获取当前地图怪物数据
$result = mysqli_query($db,"SELECT * FROM guaiwu WHERE userid='".$user['id']."' and map='".$user['map']."' and leixing='".$wg_leixing."' and qixue>'0'");
$zhandou_guaiwu=NULL;
while($row = mysqli_fetch_array($result)){
...
$yaopin_html=yaopin(kj1,$user[kj1],pvp)."|".yaopin(kj2,$user[kj2],pvp)."|".yaopin(kj3,$user[kj3],pvp)."|".yaopin(kj4,$user[kj4],pvp)."|".yaopin(kj5,$user[kj5],pvp);
$jineng_html=jineng(kj6,$user[kj6],$user[zhongzu],pvp)."|".jineng(kj7,$user[kj7],$user[zhongzu],pvp)."|".jineng(kj8,$user[kj8],$user[zhongzu],pvp)."|".jineng(kj9,$user[kj9],$user[zhongzu],pvp)."|".jineng(kj10,$user[kj10],$user[zhongzu],pvp);
...
$zidong<a href='pvp.do?go=bz&md5=$_G[md5]'>捕捉</a> <a href='pvp.do?go=tp&md5=$_G[md5]'>逃跑</a> 
<br/>技能：<a href='pvp.do?go=putong&md5=$_G[md5]'>普通</a>$jineng_html
