...
/********************************************
 wap框架头部变量
 *******************************************/
echo $wapwork->title(神话Q传);

//检测推荐人
$tuijian=$_GET['tuijian'];
if($tuijian){
if(preg_match('/^[0-9]+$/u',$tuijian)) {
$resultl = mysqli_query($db,"SELECT * FROM user WHERE id='".$tuijian."'");
$tuijianmy = mysqli_fetch_array($resultl);
if ($tuijianmy){
//推荐人存在
$_SESSION['tuijian']=$tuijian;
}
}
}
if ($my=="ok"){
...
if ($my){
echo "账号已经被注册了！<br/><a href='/reg.php?my=reg'>返回重新注册</a><br/>";
}else{
$pass=md5($pass);
$s="insert into user(name,pass,map,dengji,jingyan,tuijian) values('".$name."','".$pass."','1','1','100','".$_SESSION['tuijian']."')";
$ok=mysqli_query($db,$s);
if ($ok){
echo "<b>注册成功!<br/><a href='/reg.php?my=login'>点击立刻前往登录</a></b><br/>";
}else{
echo "<b>抱歉，注册失败！！<br/><a href='/reg.php?my=reg'>返回重新注册</a></b><br/>";
}
}
}else{
echo"密码只能包含字母、数字！<br/><a href='/reg.php?my=reg'>返回重新注册</a></b><br/><br/>";  
}
}else{
echo"用户名只能包含字母、数字！<br/><a href='/reg.php?my=reg'>返回重新注册</a></b><br/><br/>"; 
}
}else{
echo "密码不得小于6位大于16位数！<br/><a href='/reg.php?my=reg'>返回重新注册</a><br/>";
}
}else{
echo "用户名不得小于6位数大于16位数！<br/><a href='/reg.php?my=reg'>返回重新注册</a><br/>";
}
}else{
echo "验证码错误<br/><a href='/reg.php?my=reg'>返回重新注册</a><br/>";
}
...
if ($my=="ko"){
  ...
  $pass=md5($pass);
@$resultlee = mysqli_query($db,"SELECT * FROM user WHERE name='".$name."' and pass='".$pass."'");
...
echo "登录失败<br/>请检查密码账号是否正确<br/><a href='/reg.php?my=login'>点击重新登录</a><br/>";
}
}else{
echo "登录失败<br/>验证码输入错误<br/><a href='/reg.php?my=login'>点击重新登录</a><br/>";
...
echo "登录失败<br/>请检查密码手机号是否正确<br/><a href='/reg.php?my=mblogin'>点击重新登录</a><br/>";
}
}
...
echo "<br/><a href='/reg.php?my=login'>登录游戏</a>";
echo " | <a href='/reg.php?my=reg'>注册账号</a><br/><img src='/img/guanfang.jpg'/>";
//获取公告
//获取当前地图NPC
...
echo "<br/> <img src='/img/zhiyin.jpg'/><br/><a href='/reg.php?my=bj'>游戏背景</a>|<a href='/reg.php?my=wf'>游戏玩法</a><br/><a href='/reg.php?my=cw'>宠物神兵</a>";
...
echo '<form name="form1" method="post" action="reg.php?my=ok" >
...
echo "<br/><a href='/reg.php?my=login'>登录游戏</a>";
echo " | <a href='/reg.php?my=reg'>注册通行证</a>";
...
echo '<form name="form1" method="post" action="reg.php?my=ko" >
...
echo "<br/><a href='/reg.php?my=reg'>注册账号</a>";
echo " |<a href='/reg.php?my=mblogin'>手机号登录</a>";

}
if ($my=="mblogin"){
echo"<img src='/Logo.jpg'  alt='魔神传说' /><br/>";
echo " 欢迎您的到来，赶快注册账号和好友一起称霸神话世界吧！<br/>";
echo '<form name="form1" method="post" action="reg.php?my=mbko" >
...
echo "<br/><a href='/reg.php?my=reg'>注册账号</a>";
echo " |<a href='/reg.php?my=login'>账号登录</a>";
...
if($my=="wangjiko"){
	...
	$zone_wangji=rand(1000,9999);//生成随机验证码
  $sql2="update user set wangji='".$zone_wangji."' where id='".$cid."'";
$ok=mysqli_query($db,$sql2);
  //请求发送短信
  if(!$_COOKIE["time"]){
$_apiurl = "https://s.lxv.cn/bbs/aliyun-dysms-php-sdk/api.php?md5=55d1v1s55D81d8D445d484D1584f448DF415485D4D8D8F8Ff84f8151F5F48f51Ff84f5F54f8F5F51F5F8G484FD54f58fg5fg15G48F5f15f5g45f15f15g5f45f1g5g5f45f15F1F8f4f48548F151f5njmiuhduiDHUhudHDdihduda21&code=$zone_wangji&shouji=$shouji";
$r =GetPostData($_apiurl, '', 10);
	$rs = "验证码发送成功!";
	setcookie('time',TRUE,time()+120);
  }else{   
    $rs ="发送验证码失败！请勿重复发送。请120秒后重试！";
  }
  echo $rs;
		}else{
			//验证码错误
...
echo '<form name="form1" method="post" action="reg.php?my=wangjiko" >
...
echo  "<a href='/user.php?id=$xtuser[id]'>$xtuser[username]</a>$row[text]<br/>";
}
echo " <br/><a href='/reg.php'>游戏首页</a>";
setcookie("zdllq", "Alex Portr", time()+360000);
