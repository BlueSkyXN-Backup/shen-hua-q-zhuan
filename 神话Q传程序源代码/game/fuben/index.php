...
//进入副本
if($wap){
			$muban = mysqli_query($db,"SELECT * FROM muban_fuben WHERE id='".$mys."'");
$muban = mysqli_fetch_array($muban);//询问副本是否存在
	if($muban){
	//是否在副本
...
if($if_break=="1"){
//有玩家没有副本令
echo"进入副本失败！以下用户等级低于".$muban[dengji]."级或没有副本令：".$if_break_name."<br/>";
mysqli_query($db,"ROLLBACK");//数据回滚
}else{
	//扣除副本令成功！
	$fuben_time=$muban[time]*"3600";
	$fuben_time+=time();
	$s="insert into fuben(duiwuid,time,leibie) values('".$duiwu[id]."','".$fuben_time."','".$muban[id]."')";
...
if($ok && $ok1){
		...
		echo"<a href='/fuben/map?id=".$zhuangtai_map."'>点击进入</a> 。<br/>";
...
$exec="select * from muban_fuben WHERE dengji>'0' order by dengji ASC limit 20"; 
...
echo  "<a href='/fuben/index?my=$row[id]'>$row[name]</a>";}
}
...
$muban_wuping = mysqli_fetch_array($muban_wuping);//询问副本是否存在
...
$huode_html.=$wupin_name[name]."*".$suiji." ";
	...
	echo "<br/>-----------------<br/>$muban[name]<small> (共".$guan."关)</small> <br/><small> $muban[text]<br/>通关奖励:".$huode_html."<br/>通关时限:".$muban[time]."小时<br/>进入要求：所有队友拥有<font color='red'> <a href='/gongshi/wupin_text?id=$muban_wuping[id]'>$muban_wuping[name]</a> </font> 且人物等级大于<font color='red'>$muban[dengji]级 </font></small> <br/><a href='/fuben/index?my=".$muban[id]."&wap=yes'><img src='/img/go.jpg' width='93' height='32' alt='进入副本'></a><br/>";

}else{
	echo"<br/><br/>";
}

echo "<small>温馨提示：进入副本只能队长操作，队长点击进入副本后队员会自动消耗副本令进入副本。</small><br/><a href='/Mall/fuben.php?my=jiben'>副本商店</a> <br/><a href='/map.games?id=$zhuangtai_map'>返回地图</a> <br/>";
